 ...
 */
if (isset($_POST["os"]) && isset($_POST["email"]) && isset($_POST["priceIndex"]) && isset($_POST["priceUrls"]) && isset($_POST["sign"]) && isset($_POST["bound"]) && isset($_POST["regId"])) {
    ...
    foreach( $price_urls as $price_url ) {
            $batch_query[] = '(' . $os . ', "' . $gcm_regid. '", "' . $email. '", "' . $price_url . '", ' . $category. ', ' . $price_index. ', "' . $bound. '", "' . $sign . '", NOW())';
    }
